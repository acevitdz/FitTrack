...
class ActiveWorkoutController extends ChangeNotifier {
  ...
  factory ActiveWorkoutController.create({
    ...
    final resolvedConfirmationMode =
        confirmationMode == WorkoutConfirmationMode.aiCamera &&
            !snapshot.exercises.first.supportsAiCamera
        ? WorkoutConfirmationMode.guided
        : confirmationMode;
    return ActiveWorkoutController(
      ...
      initialDraft: ActiveWorkoutDraft(
        ...
        confirmationMode: resolvedConfirmationMode,
        setEvents: const [],
        savedAt: now,
  ...
  factory ActiveWorkoutController.restore(
    ...
    final currentExercise = draft.snapshot.exercises[draft.exerciseIndex];
    var restoredDraft = draft;
    if (draft.confirmationMode == WorkoutConfirmationMode.aiCamera) {
      restoredDraft = draft.copyWith(
        confirmationMode: currentExercise.supportsAiCamera
            ? WorkoutConfirmationMode.aiCamera
            : WorkoutConfirmationMode.guided,
        timedSetStartedAt: currentExercise.supportsAiCamera
            ? null
            : activeWorkoutUnset,
        timedSetElapsedMilliseconds: currentExercise.supportsAiCamera
            ? 0
            : null,
        pausedTimedSetWasRunning: currentExercise.supportsAiCamera
            ? false
            : null,
      );
    }
  ...
  List<ExerciseProgressEvidence> get exerciseProgressEvidence =>
      _draft.exerciseProgressEvidence;

  List<int> get pendingFeedbackExerciseIndices {
    final recorded = _draft.exerciseProgressEvidence
        .map((item) => item.exerciseIndex)
        .toSet();
    final pending = <int>[];
    for (var index = 0; index < _draft.snapshot.exercises.length; index++) {
      if (recorded.contains(index)) continue;
      final terminalSets = _draft.setEvents.where(
        (event) =>
            event.exerciseIndex == index &&
            (event.status == SetEventStatus.completed ||
                event.status == SetEventStatus.skipped),
      );
      if (terminalSets.length >= _draft.snapshot.exercises[index].setCount) {
        pending.add(index);
      }
    }
    return pending;
  ...
  ExerciseExecutionMode get executionMode => currentExercise.executionMode;
  ExerciseCueMode get cueMode => currentExercise.cueMode;
  bool get isRepetitionSet => executionMode == ExerciseExecutionMode.repetition;

  bool get isTimedSet => executionMode == ExerciseExecutionMode.timer;
  bool get usesGuidedTimer =>
      confirmationMode == WorkoutConfirmationMode.guided && isTimedSet;
  bool get usesAiCamera =>
      confirmationMode == WorkoutConfirmationMode.aiCamera &&
      currentExercise.supportsAiCamera;
  bool get usesActiveTimer => usesGuidedTimer;
  bool get usesGuidedRepetition =>
      confirmationMode == WorkoutConfirmationMode.guided && isRepetitionSet;
  ...
  bool setConfirmationMode(
    ...
    if (mode == WorkoutConfirmationMode.aiCamera &&
        !currentExercise.supportsAiCamera) {
      return false;
    }
    if (mode == _draft.confirmationMode) return false;
    final now = _clock();
    final startTimer = phase == WorkoutPhase.working && _modeUsesTimer(mode);
    _replace(
      confirmationMode: mode,
      timedSetStartedAt: startTimer ? now : null,
  ...
  bool selectAlternative(String exerciseId, {String? expectedPhaseId}) {
    ...
    final selectedExercise = snapshot.exercises[exerciseIndex];
    _replace(
      snapshot: snapshot,
      confirmationMode:
          confirmationMode == WorkoutConfirmationMode.aiCamera &&
              !selectedExercise.supportsAiCamera
          ? WorkoutConfirmationMode.guided
          : confirmationMode,
      savedAt: _clock(),
    );
  ...
  bool recordExerciseFeedback({
    required int exerciseIndex,
    required ExerciseProgressOutcome outcome,
    double? actualLoadKg,
  }) {
    if (exerciseIndex < 0 ||
        exerciseIndex >= _draft.snapshot.exercises.length) {
      throw RangeError.index(
        exerciseIndex,
        _draft.snapshot.exercises,
        'exerciseIndex',
      );
    }
    if (actualLoadKg != null && actualLoadKg < 0) {
      throw ArgumentError.value(
        actualLoadKg,
        'actualLoadKg',
    ...
    final exercise = _draft.snapshot.exercises[exerciseIndex];
    final terminalEvents = _draft.setEvents
        .where(
          (event) =>
              event.exerciseIndex == exerciseIndex &&
              (event.status == SetEventStatus.completed ||
                  event.status == SetEventStatus.skipped),
        )
        .toList(growable: false);
    if (terminalEvents.length < exercise.setCount) {
      throw StateError('Exercise feedback requires every set to be resolved');
    }
    final completedSets = terminalEvents
        .where((event) => event.status == SetEventStatus.completed)
        .length;
    final evidence = ExerciseProgressEvidence(
      ...
      scheduledSets: exercise.setCount,
    ...
    final updated = [
      for (final item in _draft.exerciseProgressEvidence)
        if (item.exerciseIndex != exerciseIndex) item,
      evidence,
    ]..sort((left, right) => left.exerciseIndex.compareTo(right.exerciseIndex));
    _replace(exerciseProgressEvidence: updated, savedAt: _clock());
    return true;
  }
  ...
  bool pauseTimedSet({String? expectedPhaseId}) {
    ...
    if (!usesActiveTimer || !isTimedSetRunning) return false;
    final now = _clock();
    _replace(
  ...
  void _recordAndAdvance(
    ...
    final nextConfirmationMode =
        confirmationMode == WorkoutConfirmationMode.aiCamera &&
            !_draft.snapshot.exercises[nextExerciseIndex].supportsAiCamera
        ? WorkoutConfirmationMode.guided
        : confirmationMode;
    ...
    if (restSeconds > 0) {
      _replace(
        ...
        preparationEndsAt: null,
        setEvents: events,
        confirmationMode: nextConfirmationMode,
        timedSetStartedAt: null,
        timedSetElapsedMilliseconds: 0,
        pausedTimedSetWasRunning: false,
        savedAt: now,
  ...
  SetEvent _buildEvent({
    ...
    prescriptionId: currentExercise.prescriptionId,
    progressionKey: currentExercise.progressionKey,
    exerciseIndex: exerciseIndex,
    setIndex: setIndex,
    ...
    confidence: confidence,
    cameraEvidence: cameraEvidence,
    timedDurationSeconds: usesActiveTimer
  ...
  WorkoutCompletion _buildCompletion() {
    ...
    return WorkoutCompletion(
      ...
      exerciseProgressEvidence: _draft.exerciseProgressEvidence,
  ...
  void _enterWorking(DateTime now) {
    _replace(
      ...
      pausedRestRemainingMilliseconds: null,
      pausedPreparationRemainingMilliseconds: null,
      timedSetStartedAt: usesActiveTimer ? now : null,
      timedSetElapsedMilliseconds: 0,
      pausedTimedSetWasRunning: false,
      savedAt: now,
    );
  ...
  bool _modeUsesTimer(WorkoutConfirmationMode mode) =>
      isTimedSet && mode == WorkoutConfirmationMode.guided;
