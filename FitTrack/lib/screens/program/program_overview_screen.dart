                ...
                Card(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          state.displayProgramTitle(version, program: program),
...
class _SessionTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      child: ExpansionTile(
        ...
        title: Text(AppState.displaySessionTitle(session.title)),
        subtitle: Text(
              ...
              child: Column(
                ...
                children: [
                  ...
                  for (final item in [
                    ...
                    _prescriptionTile(context, block, item),
                ],
              ),
            ),
        ],
  ...
  Widget _prescriptionTile(
    ...
    final decision = occurrence == null
        ? null
    ...
    final targetText = target == null
        ? '${prescription.sets} hiệp • ${prescription.targetLabel}'
        : '${target.sets} hiệp • ${_targetLabel(target, prescription.perSide)}';
    return Padding(
      padding: const EdgeInsets.only(bottom: 6),
      child: ListTile(
        dense: true,
        isThreeLine: decision != null,
        contentPadding: EdgeInsets.zero,
        leading: const Icon(Icons.fitness_center, size: 19),
        title: Text(
          prescription.exerciseName ??
              state.resolveExerciseName(prescription.exerciseId),
        ),
        subtitle: Text(
          decision == null
              ? targetText
              : '$targetText\nCá nhân hóa: ${_decisionLabel(decision.kind)}',
        ),
      ),
    );
  }

  String _targetLabel(ProgressionTarget target, bool perSide) {
    final value = target.minimum == target.maximum
        ? '${target.minimum}'
        : '${target.minimum}-${target.maximum}';
    final side = perSide ? '/bên' : '';
    if (target.targetType == PrescriptionTargetType.repetitions) {
      return '$value lần$side';
    }
    if (target.minimum % 60 == 0 && target.maximum % 60 == 0) {
      final minimum = target.minimum ~/ 60;
      final maximum = target.maximum ~/ 60;
      return '${minimum == maximum ? minimum : '$minimum-$maximum'} phút$side';
    }
    return '$value giây$side';
  }

  String _decisionLabel(ProgressionDecisionKind kind) => switch (kind) {
    ProgressionDecisionKind.increase => 'tăng nhẹ một bước',
    ProgressionDecisionKind.hold => 'giữ nguyên',
    ProgressionDecisionKind.regress => 'giảm một bước',
    ProgressionDecisionKind.safetyHold => 'giữ mức an toàn',
    ProgressionDecisionKind.deload => 'tuần giảm tải',
  };
