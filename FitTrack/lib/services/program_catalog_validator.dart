...
import 'execution_config_resolver.dart';
        ...
        for (final block in [
          ...
          for (final prescription in block.prescriptions) {
            ...
            if (!activeExercises.containsKey(prescription.exerciseId)) {
              ...
              final exercise = activeExercises[prescription.exerciseId]!;
              final config = resolveExecutionConfig(
                prescription: prescription,
                exercise: exercise,
              );
              if (config.executionMode == ExerciseExecutionMode.timer &&
                  config.cueMode != ExerciseCueMode.countdown) {
                errors.add('execution_mode_cue_mismatch:${prescription.id}');
              }
              if (config.executionMode == ExerciseExecutionMode.repetition &&
                  config.cueMode != ExerciseCueMode.voice) {
                errors.add('execution_mode_cue_mismatch:${prescription.id}');
              }
            }
            ...
            if (poseRule != null &&
                (prescription.exerciseId != 'squat' ||
                    poseRule != 'squat_pose_v1')) {
              errors.add('pose_rule_exercise_mismatch:${prescription.id}');
            }
            if (prescription.cameraTargetReps != null &&
                (prescription.exerciseId != 'squat' ||
                    poseRule != 'squat_pose_v1')) {
              errors.add('camera_target_exercise_mismatch:${prescription.id}');
            }
          }
        }
      }
